...
public interface EventoVirtualRepository extends CrudRepository<EventoVirtual, Long> {
    ...
    @Query("SELECT ev FROM InscripcionEventoVirtual iev JOIN iev.eventoVirtual ev WHERE ev.fechaHoraInicio >= :fechaHora " +
            "AND iev.persona.id = :personaId AND iev.estado = 'A' AND iev.eliminado = false ")
    Iterable<EventoVirtual> listProximosByPersonaInscrita(Date fechaHora, long personaId);
}